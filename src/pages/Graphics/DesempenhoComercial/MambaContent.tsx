...
export default function MambaContent() {
  const {
    data,
    error,
    loading,
  ...
  const leadsData = Object.entries(data).map(([date, values]) => {
    ...
    return {
      ...
      "Total Leads": values.LIMBOLEADS + values.LEADS,
  ...
  const reunioesAgendadasData = Object.entries(data).map(([date, values]) => {
    const totalReunioesDia = Object.values(
      values.Reunioes_Agendadas as { [key: string]: number },
    ).reduce(
      (a, b) => a + b,
      0, // Valor inicial
    );
  ...
  const reunioesRealizadasData = Object.entries(data).map(([date, values]) => {
    const totalReunioesDia = Object.values(
      values.Reunioes_Realizadas as { [key: string]: number },
    ).reduce(
      (a, b) => a + b,
      0, // Valor inicial
    );
